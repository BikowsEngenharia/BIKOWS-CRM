/* ==========================================
   ...
   DRE · Recebimentos · Custos · Parceiros
   ========================================== */
const ProjetoFinanceiro = (() => {
  ...
  function calcDRE(p) {
    ...
    // Custos de horas (do campo básico do projeto)
    const custoHoras = (p.horasTrabalhadas || 0) * (p.valorHora || 0);

    // Custos diretos lançados no financeiro do projeto
    const custosLancados = (fin.custos || []).reduce((s, c) => s + (c.valor || 0), 0);

    // Custos diretos do campo básico (compatibilidade)
    const custosBasico = p.custosDirectos || 0;

    // Total custos (evita duplicidade: se há custos lançados, usa eles; senão usa o campo básico)
    ...
    // Parceiros / comissões (calculadas sobre lucro bruto)
  ...
  function open(projetoId) {
    ...
    Modal.open({
      title: `💰 Financeiro — ${Utils.escHtml(p.titulo)}`,
      size: 'modal-xl',
      body: _buildBody(p),
      saveLabel: null,
      cancelLabel: 'Fechar',
  ...
  function _renderDRE(p, dre, margemColor) {
    const rows = [
      ...
      { label: '(-) Comissões / Parceiros',  val: -dre.totalComissoes, small: true, color: '#f59e0b' },
      { label: '(=) Lucro Líquido',          val: dre.lucroLiquido,    bold: true,  sep: true, color: margemColor },
    ];
    ...
    const kpis = [
      ...
      { label: 'Lucro Líquido', val: Utils.formatCurrency(dre.lucroLiquido), color: margemColor },
    ...
    return `
      ...
      <div style="background:var(--bg);border-radius:var(--radius);padding:16px">
        <div class="font-bold text-sm mb-3">📄 Demonstrativo de Resultado</div>
        ${rows.map(r => `
          <div style="display:flex;justify-content:space-between;align-items:center;
            ...
            <span style="font-weight:${r.bold ? '700' : '500'};color:${r.color || 'var(--text)'};font-size:${r.bold ? '15px' : '13px'}">
              ${Utils.formatCurrency(Math.abs(r.val))}${r.val < 0 ? ' ↓' : ''}
            </span>
          </div>`).join('')}
      </div>
      ...
      <div style="margin-top:16px">
        <div class="flex items-center justify-between mb-1">
          <span class="text-xs text-muted">Margem Líquida</span>
        ...
        <div class="text-xs text-muted mt-1">
          ${dre.margemPct >= 40 ? '✅ Excelente margem' : dre.margemPct >= 20 ? '⚠ Margem aceitável — revise custos' : '🔴 Margem baixa — atenção aos custos'}
        </div>
      </div>
      ...
      ${dre.custoTotal === 0 && (dre.fin?.custos||[]).length === 0 ? `
        <div style="margin-top:12px;padding:10px;background:#fef9c3;border-radius:var(--radius);font-size:13px;color:#854d0e">
          ℹ Para ver a margem real, registre horas trabalhadas no projeto ou lance custos na aba "Custos".
        </div>` : ''}
    `;
  }
  ...
  function _renderRecebimentos(p, fin, dre) {
    ...
    return `
      <div class="flex items-center justify-between mb-3">
        <div>
          <div class="font-bold">Parcelas / Medições</div>
          <div class="text-xs text-muted">Valor do contrato: ${Utils.formatCurrency(p.valor)} |
            ...
            <span style="color:${Math.abs(diff)<0.01?'#10b981':'#ef4444'};font-weight:600">
              ${Math.abs(diff) < 0.01 ? '✅ Balanceado' : `Falta: ${Utils.formatCurrency(diff)}`}
            </span>
          </div>
        </div>
      ...
      ${recs.length === 0 ? `
        ...
        <table class="tbl">
          <thead>
            <tr><th>#</th><th>Descrição</th><th>Valor</th><th>Vencimento</th><th>Pagamento</th><th>Status</th><th>Financeiro</th><th>Ações</th></tr>
          </thead>
          <tbody>
            ${recs.map((r, i) => {
              ...
              return `<tr>
                ...
                <td class="text-sm ${atrasado ? 'text-danger' : ''}">
                  ...
                  ${atrasado ? `<div class="text-xs">⚠ ${Math.abs(dias)}d atraso</div>` : ''}
                </td>
                <td class="text-xs text-muted">${Utils.escHtml(r.formaPagamento || '—')}</td>
                <td>${r.status === 'recebido'
                  ? `<span class="badge badge-green">✅ Recebido</span><div class="text-xs text-muted">${Utils.formatDate(r.dataRecebimento)}</div>`
                  : `<span class="badge badge-yellow">⏳ Pendente</span>`}</td>
                <td>${r.lancadoFinanceiro
                  ? `<span class="badge badge-green text-xs">✅ Lançado</span>`
                  : `<span class="badge badge-gray text-xs">Não lançado</span>`}</td>
                <td>
                  <div class="tbl-actions">
                    ${r.status !== 'recebido' ? `
                      <button class="btn btn-xs btn-primary" onclick="ProjetoFinanceiro.marcarRecebido('${p.id}','${r.id}')">✅ Receber</button>
                    ` : ''}
                    ${!r.lancadoFinanceiro ? `
                      <button class="btn btn-xs btn-secondary" onclick="ProjetoFinanceiro.lancarFinanceiro('${p.id}','${r.id}')">📤 Lançar</button>
                    ` : ''}
                    <button class="btn btn-xs btn-danger" onclick="ProjetoFinanceiro.removeRecebimento('${p.id}','${r.id}')">🗑</button>
                  </div>
                </td>
              </tr>`;
            }).join('')}
        ...
        <div style="margin-top:12px;padding:10px;background:var(--bg);border-radius:var(--radius);font-size:13px">
          💡 <strong>Lançar no Financeiro:</strong> gera um recebível no controle geral da empresa com a data de vencimento configurada.
    ...
    return `
      <div class="flex items-center justify-between mb-3">
        <div>
          ...
          <div class="text-xs text-muted">Total lançado: <strong>${Utils.formatCurrency(total)}</strong></div>
        </div>
      ...
      ${custos.length === 0 ? `
        <div class="empty-state" style="padding:30px">
          <div class="empty-icon">📦</div>
          <div>Nenhum custo lançado neste projeto.</div>
          <div class="text-xs text-muted mt-1">Lance deslocamentos, materiais, subcontratações e outros custos diretos.</div>
        </div>` : `
        <table class="tbl">
          <thead>
            <tr><th>Descrição</th><th>Categoria</th><th>Fornecedor</th><th>Data</th><th>Valor</th><th>Financeiro</th><th>Ações</th></tr>
          </thead>
          <tbody>
            ${custos.map(c => `<tr>
              <td class="font-bold text-sm">${Utils.escHtml(c.descricao)}</td>
              <td class="text-xs text-muted">${Utils.escHtml(c.categoria || '—')}</td>
              <td class="text-sm">${Utils.escHtml(c.fornecedor || '—')}</td>
              ...
              <td>
                <div class="tbl-actions">
                  ${!c.lancadoFinanceiro ? `
                    <button class="btn btn-xs btn-secondary" onclick="ProjetoFinanceiro.lancarCustoFinanceiro('${p.id}','${c.id}')">📤 Lançar</button>
                  ` : ''}
                  <button class="btn btn-xs btn-danger" onclick="ProjetoFinanceiro.removeCusto('${p.id}','${c.id}')">🗑</button>
                </div>
              </td>
            </tr>`).join('')}
          </tbody>
        ...
        <div style="margin-top:8px;font-size:13px;color:var(--text-muted)">
          💡 <strong>Lançar no Financeiro:</strong> registra como conta a pagar no controle geral da empresa,
  ...
  function _renderParceiros(p, fin, dre) {
    ...
    return `
      <div class="flex items-center justify-between mb-3">
        <div>
          <div class="font-bold">Parceiros / Comissões</div>
          <div class="text-xs text-muted">Comissões calculadas sobre o lucro bruto do projeto</div>
        </div>
        <button class="btn btn-primary btn-sm" onclick="ProjetoFinanceiro.openAddParceiro('${p.id}')">+ Parceiro</button>
      </div>

      <!-- Aviso de base de cálculo -->
      <div style="background:#f0fdf4;border:1px solid #86efac;border-radius:var(--radius);padding:10px 14px;margin-bottom:14px;font-size:13px">
        📐 <strong>Base de cálculo:</strong> Lucro Bruto = ${Utils.formatCurrency(dre.lucroBruto)}
        (Receita ${Utils.formatCurrency(dre.receita)} − Custos ${Utils.formatCurrency(dre.custoTotal)})
      </div>
      ...
      ${parceiros.length === 0 ? `
        <div class="empty-state" style="padding:30px">
          ...
          <div class="text-xs text-muted mt-1">Adicione parceiros com comissão % sobre lucro bruto ou valor fixo.</div>
        </div>` : `
        <table class="tbl">
          <thead>
            <tr><th>Nome / Empresa</th><th>Tipo</th><th>Base / Valor</th><th>Valor a Pagar</th><th>Vencimento</th><th>Status</th><th>Financeiro</th><th>Ações</th></tr>
          </thead>
          <tbody>
            ${parceiros.map(par => {
              ...
              return `<tr>
                ...
                <td>${par.tipo === 'comissao'
                  ? `<span class="badge badge-purple">% Comissão</span>`
                ...
                <td class="text-sm text-muted">${Utils.formatDate(par.vencimento) || '—'}</td>
                <td>${par.status === 'pago'
                ...
                <td>
                  <div class="tbl-actions">
                    ${par.status !== 'pago' ? `
                      <button class="btn btn-xs btn-primary" onclick="ProjetoFinanceiro.marcarParceiroPago('${p.id}','${par.id}')">✅ Pagar</button>
                    ` : ''}
                    ${!par.lancadoFinanceiro ? `
                      <button class="btn btn-xs btn-secondary" onclick="ProjetoFinanceiro.lancarParceiroFinanceiro('${p.id}','${par.id}')">📤 Lançar</button>
                    ` : ''}
                    <button class="btn btn-xs btn-danger" onclick="ProjetoFinanceiro.removeParceiro('${p.id}','${par.id}')">🗑</button>
                  </div>
                </td>
              </tr>`;
            }).join('')}
        ...
        <div class="flex justify-between mt-3 font-bold" style="padding:10px;background:var(--bg);border-radius:var(--radius)">
          <span>Total de Comissões / Pagamentos</span>
  ...
  function openAddRecebimento(projetoId) {
    ...
    Modal.open({
      title: '+ Nova Parcela / Medição',
      size: 'modal-sm',
      body: `
        <div class="form-group">
          <label class="form-label">Descrição</label>
          <input class="form-control" id="pfRecDescricao" value="Parcela ${idx}" placeholder="Ex: Parcela 1, Medição Final...">
        </div>
        <div class="form-row">
          <div class="form-group">
        ...
        <label style="display:flex;align-items:center;gap:8px;cursor:pointer;margin-top:4px">
          ...
          <span class="text-sm">Lançar automaticamente no financeiro geral (Contas a Receber)</span>
        </label>
      `,
  ...
  function marcarRecebido(projetoId, recId) {
    ...
    Modal.open({
      ...
      size: 'modal-sm',
      body: `
        <p class="text-sm mb-3">Confirme a data de recebimento de <strong>${Utils.formatCurrency(rec.valor)}</strong> — ${Utils.escHtml(rec.descricao)}.</p>
        <div class="form-row">
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <select class="form-control" id="pfFormaReceb">
              ...
              <option value="Transferência" ${rec.formaPagamento==='Transferência'?'selected':''}>Transferência</option>
        ...
        <label style="display:flex;align-items:center;gap:8px;cursor:pointer">
          ...
          <span class="text-sm">Registrar no caixa geral (Lançamentos)</span>
        </label>
      `,
      saveCb: () => {
  ...
  function openAddCusto(projetoId) {
    Modal.open({
      ...
      body: `
        <div class="form-group">
          <label class="form-label">Descrição *</label>
          <input class="form-control" id="pfCustoDesc" placeholder="Ex: Deslocamento, Material, Subcontratação...">
        </div>
        <div class="form-row">
          <div class="form-group">
            <label class="form-label">Valor (R$) *</label>
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <select class="form-control" id="pfCustoCat">
              <option value="">—</option>
              <option value="Deslocamento">Deslocamento / Combustível</option>
              <option value="Hospedagem">Hospedagem / Alimentação</option>
              <option value="Materiais">Materiais / Equipamentos</option>
              <option value="Subcontratação">Subcontratação</option>
        ...
        <label style="display:flex;align-items:center;gap:8px;cursor:pointer;margin-top:4px">
          ...
          <span class="text-sm">Lançar como despesa no financeiro geral (Contas a Pagar)</span>
        </label>
      `,
  ...
  function _saveCusto(projetoId) {
    ...
    if (lancar) {
      DB.create('contaspagar', {
        ...
        descricao: `${p.titulo} — ${custo.descricao}`,
        valor: custo.valor,
  ...
  function lancarCustoFinanceiro(projetoId, custoId) {
    ...
    custo.lancadoFinanceiro = true;
    _save(projetoId, fin);
    Toast.success('Custo lançado no financeiro geral!');
    _refresh(projetoId);
  }
  ...
  function openAddParceiro(projetoId) {
    ...
    Modal.open({
      title: '+ Novo Parceiro / Comissão',
      size: 'modal-sm',
      body: `
        <div class="form-group">
          <label class="form-label">Nome / Empresa *</label>
          <input class="form-control" id="pfParNome" placeholder="Ex: Eng. João Silva, Empresa XYZ">
        </div>
        <div class="form-group">
          ...
          <select class="form-control" id="pfParTipo" onchange="ProjetoFinanceiro._toggleTipoParceiro()">
            <option value="comissao">% Comissão sobre Lucro Bruto</option>
        ...
        <div id="pfParComissaoGrp">
          ...
          <div style="background:#f0fdf4;border-radius:var(--radius);padding:8px 12px;font-size:13px;margin-bottom:12px">
            💡 Lucro Bruto atual: <strong>${Utils.formatCurrency(dre.lucroBruto)}</strong>
            — valor será recalculado ao fechar o projeto.
          </div>
        </div>
        ...
        <label style="display:flex;align-items:center;gap:8px;cursor:pointer;margin-top:4px">
          ...
          <span class="text-sm">Lançar como conta a pagar no financeiro geral</span>
        </label>
      `,
  ...
  function _saveParceiro(projetoId) {
    ...
    if (lancar && valorPagar > 0 && vencimento) {
      DB.create('contaspagar', {
        fornecedor: nome,
        descricao: `Comissão/Parceria — ${p.titulo}`,
        valor: valorPagar,
        vencimento,
        status: 'pendente',
        categoria: 'Comissões / Parcerias',
        projetoId,
        origem: 'projeto_financeiro',
      });
  ...
  function marcarParceiroPago(projetoId, parceiroId) {
    ...
    Modal.open({
      title: '✅ Registrar Pagamento ao Parceiro',
      size: 'modal-sm',
      body: `
        ...
        <label style="display:flex;align-items:center;gap:8px;cursor:pointer">
          ...
          <span class="text-sm">Registrar saída no caixa geral (Lançamentos)</span>
        </label>
      `,
      saveCb: () => {
        ...
        if (lancar) {
          DB.create('lancamentos', {
            descricao: `Comissão ${par.nome} — ${p.titulo}`,
            valor: valorPagar,
            tipo: 'despesa',
            data,
            categoria: 'Comissões / Parcerias',
  ...
  function lancarParceiroFinanceiro(projetoId, parceiroId) {
    ...
    if (!valorPagar) { Toast.error('Valor zerado — registre os custos do projeto primeiro.'); return; }
    if (!par.vencimento) { Toast.error('Defina a data de pagamento do parceiro antes de lançar.'); return; }
    ...
    par.lancadoFinanceiro = true;
    _save(projetoId, fin);
    Toast.success('Comissão lançada no financeiro geral!');
    _refresh(projetoId);
  }
  ...
  /* ====================================================
     LANÇAMENTO INICIAL NO FINANCEIRO GERAL
     (chamado quando status → em_andamento/contratado)
     ==================================================== */
  function sugerirConfiguracaoPagamentos(projetoId) {
    const p = _get(projetoId);
    if (!p || !p.valor) return;
    const fin = _fin(p);
    if ((fin.recebimentos || []).length > 0) return; // já configurado

    setTimeout(() => {
      Toast.info(
        `💰 Projeto "<strong>${Utils.escHtml(p.titulo)}</strong>" em andamento. ` +
        `<a href="#" onclick="ProjetoFinanceiro.open('${projetoId}');return false;" style="color:var(--primary);font-weight:600">` +
        `Configure o cronograma de recebimentos →</a>`,
        8000
      );
    }, 800);
  }
