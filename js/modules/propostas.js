/* ==========================================
   PROPOSTAS â€” OrÃ§amentos e propostas comerciais
   ========================================== */
const Propostas = (() => {
  ...
  let _filter = { status: '' };
  let _formItems = []; // itens da proposta em ediÃ§Ã£o

  /* ------------------------------------------------
     Gerador de nÃºmero de proposta â€” PROP-YYYY-NNNNN
     Busca o maior sequencial existente e incrementa,
     nunca depende do .length (quebra com exclusÃµes)
  ------------------------------------------------ */
  // Piso histÃ³rico â€” Ãºltima proposta emitida antes do sistema
  ...
  function _nextNumeroProposta() {
    ...
    let max = _PISO_SEQUENCIA; // comeÃ§a do piso, nunca abaixo
    todas.forEach(p => {
    ...
    // TambÃ©m considera nÃºmeros no formato legado BIK-YYYY-CTR-NNN
  ...
  function _renderItemsSection() {
    ...
    body.innerHTML = _formItems.map((it, i) => `
      <tr>
        <td><input class="form-control" style="min-width:160px" value="${Utils.escHtml(it.desc)}" oninput="Propostas._setItemField(${i},'desc',this.value)" placeholder="DescriÃ§Ã£o do serviÃ§o"></td>
        ...
        <td><button class="btn btn-xs btn-danger" onclick="Propostas.removeItemRow(${i})">âœ•</button></td>
      </tr>`).join('');
    _updateItemsTotal();
  }
    ...
    document.getElementById('pageContent').innerHTML = `
      ...
      <div class="card">
        ...
        <div class="table-wrap">
          ...
          <table class="tbl">
            <thead><tr><th>NÂº</th><th>Proposta</th><th>Cliente</th><th>Valor</th><th>Validade</th><th>ResponsÃ¡vel</th><th>Status</th><th>AÃ§Ãµes</th></tr></thead>
            <tbody>
              ${list.map(p => {
                const dias = Utils.daysUntil(p.validade);
                const expirado = dias != null && dias < 0 && !['aprovada','recusada'].includes(p.status);
                const validadeLabel = dias == null ? 'â€”' : dias < 0 ? `Expirada ${Math.abs(dias)}d` : dias === 0 ? 'Expira hoje' : `${dias}d restantes`;
                return `<tr>
                  <td>
                    <div style="font-weight:700;font-size:12px;color:var(--primary);white-space:nowrap">${Utils.escHtml(p.numero||'â€”')}</div>
                    <button class="btn btn-xs" style="margin-top:3px;padding:1px 6px;font-size:10px;color:var(--text-muted);border:1px solid var(--border);background:transparent"
                      onclick="navigator.clipboard.writeText('${Utils.escHtml(p.numero||'')}').then(()=>Toast.show('NÃºmero copiado!'))"
                      title="Copiar nÃºmero">ðŸ“‹</button>
                  </td>
                  ...
                  <td class="text-sm">${Utils.escHtml(Utils.getClientName(p.clienteId) !== 'â€”' ? Utils.getClientName(p.clienteId) : (p.clienteNome || 'â€”'))}</td>
                  ...
                  <td class="text-sm">${Utils.escHtml(p.responsavel||'â€”')}</td>
                  <td>${Utils.propBadge(p.status)}</td>
                  <td>
                    <div class="tbl-actions">
                      <button class="btn btn-xs btn-secondary" onclick="Propostas.view('${p.id}')">Ver</button>
                      <button class="btn btn-xs btn-success" onclick="PropostaGenerator.open('${p.id}')" title="Gerar proposta PDF">ðŸ–¨ Gerar</button>
                      <button class="btn btn-xs btn-secondary" onclick="Propostas.duplicar('${p.id}')" title="Duplicar proposta">ðŸ“‹ Dupl.</button>
                      ${p.canvaLink ? `<a href="${Utils.escHtml(p.canvaLink)}" target="_blank" rel="noopener" class="btn btn-xs btn-secondary" title="Abrir no Canva">ðŸŽ¨</a>` : ''}
                      <button class="btn btn-xs btn-secondary" onclick="Propostas.openForm('${p.id}')">âœ</button>
                      <button class="btn btn-xs btn-danger" onclick="Propostas.deleteProposta('${p.id}')">ðŸ—‘</button>
                    </div>
                  </td>
                </tr>`;
              }).join('')}
  ...
  function emptyState() {
    return `<div class="empty-state"><div class="empty-icon">ðŸ“„</div><div class="empty-title">Nenhuma proposta</div><button class="btn btn-primary mt-4" onclick="Propostas.openForm()">+ Criar Proposta</button></div>`;
  }
    ...
    Modal.open({
      ...
      body: `
        <div style="background:linear-gradient(135deg,var(--primary),#1e40af);color:#fff;padding:16px 20px;border-radius:var(--radius);margin-bottom:20px">
          <div class="flex items-center justify-between mb-1">
            <div style="display:flex;align-items:center;gap:10px">
              <span style="font-size:22px;font-weight:800;letter-spacing:.02em">${Utils.escHtml(p.numero||'â€”')}</span>
              <button onclick="navigator.clipboard.writeText('${Utils.escHtml(p.numero||'')}').then(()=>Toast.show('NÃºmero copiado!'))"
                ...
                title="Copiar nÃºmero">ðŸ“‹ Copiar</button>
            </div>
        ...
        <div class="detail-grid mb-4">
          <div class="detail-field"><div class="detail-label">Cliente</div><div class="detail-value">${Utils.escHtml(cliente?.nome || p.clienteNome || 'â€”')}</div></div>
          <div class="detail-field"><div class="detail-label">ResponsÃ¡vel</div><div class="detail-value">${Utils.escHtml(p.responsavel||'â€”')}</div></div>
        ...
        <div class="detail-field mb-3">
          ...
          <div style="overflow-x:auto">
            <table class="tbl">
              <thead><tr><th>DescriÃ§Ã£o</th><th>Qtd</th><th>Un.</th><th>Val. Unit.</th><th>Desc%</th><th>Total</th></tr></thead>
              <tbody>
                ${p.itens.map(it => `<tr>
                  ...
                  <td class="text-sm text-center">${it.disc ? it.disc+'%' : 'â€”'}</td>
        ...
        ${p.observacoes ? `<div class="detail-field mb-3"><div class="detail-label">ObservaÃ§Ãµes</div><div class="detail-value" style="white-space:pre-wrap">${Utils.escHtml(p.observacoes)}</div></div>` : ''}
        ...
        <div class="detail-field mb-3" style="background:var(--bg);border:1px solid var(--border);border-radius:var(--radius);padding:12px">
          <div class="detail-label mb-2">ðŸŽ¨ Proposta no Canva</div>
          <div style="display:flex;align-items:center;gap:10px">
            ...
            <button onclick="navigator.clipboard.writeText('${Utils.escHtml(p.canvaLink)}').then(()=>Toast.show('Link copiado!'))"
              class="btn btn-sm btn-secondary">ðŸ“‹ Copiar link</button>
          </div>
        </div>` : ''}

        ${(p.origem === 'gerador-pdf' || p.origem === 'gerador-canva') ? `
        <div class="text-xs text-muted mb-3">Origem: ${p.origem === 'gerador-canva' ? 'ðŸŽ¨ Gerado via Canva' : 'ðŸ“„ Gerado via PDF/HTML'}</div>` : ''}

        <!-- HISTÃ“RICO DE VERSÃ•ES -->
        ${versoes.length > 0 ? `
        <div class="detail-field mb-3">
          <div class="detail-label mb-2">ðŸ“‹ HistÃ³rico de VersÃµes</div>
          <div style="display:flex;flex-direction:column;gap:6px">
            ${[...versoes].reverse().map((v, i) => `
              <div style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:var(--bg);border-radius:var(--radius);border-left:3px solid ${i===0?'var(--primary)':'var(--border)'}">
                ...
                <div style="flex:1">
                  ...
                  <div class="text-xs text-muted">${v.motivo || 'RevisÃ£o'}</div>
                </div>
            ...
            <div style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:var(--bg);border-radius:var(--radius);border-left:3px solid var(--border);opacity:0.6">
              ...
              <div style="flex:1"><div class="font-bold text-sm">VersÃ£o original</div></div>
            </div>
          </div>
        </div>` : ''}
        ...
        <div class="mt-4 flex gap-2" style="flex-wrap:wrap">
          <button class="btn btn-success btn-sm" onclick="Modal.close();PropostaGenerator.open('${id}')">ðŸ–¨ Gerar PDF</button>
          ${p.canvaLink ? `<a href="${Utils.escHtml(p.canvaLink)}" target="_blank" rel="noopener" class="btn btn-secondary btn-sm">ðŸŽ¨ Abrir no Canva</a>` : ''}
          ${p.status === 'aprovada' ? `<button class="btn btn-primary btn-sm" onclick="Modal.close();Propostas.criarRecebivel('${id}')">ðŸ’° Criar RecebÃ­vel</button>` : ''}
          ${p.status === 'aprovada' ? `<button class="btn btn-primary btn-sm" onclick="Modal.close();Contratos.criarDePropostal('${id}')">ðŸ“ Criar Contrato</button>` : ''}
          <button class="btn btn-secondary btn-sm" onclick="Modal.close();Propostas.openForm('${id}')">âœ Editar / Nova VersÃ£o</button>
    ...
    Modal.open({
      ...
      body: `
        ...
        <div style="background:var(--primary);color:#fff;border-radius:var(--radius);padding:12px 16px;margin-bottom:16px;display:flex;align-items:center;gap:12px">
          <div style="flex:1">
            <div style="font-size:11px;opacity:.75;text-transform:uppercase;letter-spacing:.05em">NÃºmero da Proposta</div>
            <input id="fpNum" value="${Utils.escHtml(nextNum)}"
              ...
              title="EditÃ¡vel se necessÃ¡rio">
          </div>
        ...
        <div class="form-group">
          <label class="form-label">TÃ­tulo da Proposta *</label>
          <input class="form-control" id="fpTitulo" value="${Utils.escHtml(p?.titulo||'')}" placeholder="Ex: AdequaÃ§Ã£o NR-12 â€” FrigorÃ­fico Bela Vista">
        </div>
        <div class="form-row">
          ...
          <div class="form-group">
            ...
            <select class="form-control" id="fpResp"><option value="">â€”</option>${respOpts}</select>
          </div>
        </div>
        <div class="form-row">
          <div class="form-group">
        ...
        <div class="form-group">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
            <label class="form-label" style="margin:0">ðŸ“‹ Itens da Proposta</label>
          ...
          <div style="overflow-x:auto">
            <table class="tbl" style="min-width:620px">
              <thead><tr><th style="min-width:160px">DescriÃ§Ã£o</th><th>Qtd</th><th>Un.</th><th>Val. UnitÃ¡rio</th><th>Desc%</th><th>Total</th><th></th></tr></thead>
        ...
        <div class="form-group">
          <label class="form-label">DescriÃ§Ã£o do Escopo</label>
          <textarea class="form-control" id="fpDesc" rows="3">${Utils.escHtml(p?.descricao||'')}</textarea>
        </div>
        <div class="form-group">
          <label class="form-label">ObservaÃ§Ãµes / CondiÃ§Ãµes de Pagamento</label>
        ...
        ${id ? `<div class="form-group" style="background:var(--bg);padding:10px;border-radius:var(--radius)">
          <label class="form-label">Motivo da revisÃ£o (se alterar o valor)</label>
          <input class="form-control" id="fpMotivoRevisao" placeholder="Ex: Ajuste de escopo, negociaÃ§Ã£o de desconto...">
        </div>` : ''}
      `,
  ...
  function saveProposta(id) {
    ...
    // Registrar versÃ£o anterior se o valor mudou
    let versoes = [];
    if (id) {
      ...
      if (existing) {
        ...
        if (existing.valor && existing.valor !== valor) {
          versoes = [...versoes, {
            ...
            motivo: document.getElementById('fpMotivoRevisao')?.value || 'RevisÃ£o de valor',
    ...
    if (id) { DB.update('propostas', id, data); Toast.success('Proposta atualizada â€” nova versÃ£o registrada'); }
  ...
  function criarRecebivel(id) {
    ...
    if (DB.getAll('recebiveis').some(r => r.propostaOrigemId === id)) {
      Toast.error('JÃ¡ existe um recebÃ­vel para esta proposta'); return;
    }
    DB.create('recebiveis', {
      clienteId: p.clienteId,
      descricao: `${p.numero || 'Proposta'} â€” ${p.titulo}`,
      valorTotal: p.valor,
      propostaOrigemId: id,
    ...
    Toast.success('RecebÃ­vel criado! Acesse Financeiro â†’ Contas a Receber para configurar as parcelas.');
    render();
  }
  ...
  /* ====================================================
     ...
     ==================================================== */
  let _etapasContrato = []; // etapas em ediÃ§Ã£o no modal de fechamento
  ...
  function abrirFluxoContratacao(propostaId) {
    ...
    Modal.open({
      title: `ðŸ¤ Fechar Contrato â€” ${Utils.escHtml(prop.titulo)}`,
      size: 'modal-xl',
      body: _buildContratoBody(prop, cli, respOpts, codSuggest),
      saveLabel: 'âœ… Confirmar e Criar Projeto',
      saveCb: () => _confirmarContrato(propostaId),
    });

    // Renderiza etapas iniciais apÃ³s o modal abrir
    ...
    return `
      ...
      <div style="background:linear-gradient(135deg,#1a56db15,#7c3aed10);border:1px solid var(--border);border-radius:var(--radius);padding:12px 16px;margin-bottom:18px;display:flex;justify-content:space-between;align-items:center">
        <div>
          ...
          <div class="font-bold">${Utils.escHtml(prop.numero||'â€”')} Â· ${Utils.escHtml(prop.titulo)}</div>
          <div class="text-sm text-muted">${Utils.escHtml(cli?.nome||'â€”')}</div>
        </div>
        <div style="text-align:right">
      ...
      <div id="cpanel-fin">
        <div class="form-row">
          ...
          <div class="form-group" style="flex:3">
            <label class="form-label">CondiÃ§Ã£o de Pagamento *</label>
            <select class="form-control" id="cCondicao" onchange="Propostas._toggleCondicao()">
              <option value="avista">ðŸ’µ Ã€ Vista</option>
              <option value="parcelas">ðŸ“… Parcelas Fixas</option>
              <option value="medicoes">ðŸ“ MediÃ§Ãµes / Vinculado a Etapas</option>
            </select>
          </div>
        </div>
        ...
        <div id="cGrpMedicoes" style="display:none;background:#f0fdf4;border:1px solid #86efac;border-radius:var(--radius);padding:12px;margin-bottom:12px;font-size:13px">
          ðŸ“ <strong>Vinculado a Etapas:</strong> configure as etapas na aba "Projeto & Escopo" e marque
          quais geram recebimento. O valor e data de vencimento de cada recebÃ­vel serÃ£o definidos lÃ¡.
        </div>

        <div class="form-group">
          <label class="form-label">Prazo de VigÃªncia do Contrato</label>
          <div class="form-row" style="margin:0">
            <div class="form-group" style="margin-bottom:0">
              <label class="form-label text-xs">Data de InÃ­cio</label>
              <input class="form-control" id="cContratoInicio" type="date" value="${Utils.todayStr()}">
            </div>
            <div class="form-group" style="margin-bottom:0">
              <label class="form-label text-xs">Data de TÃ©rmino</label>
        ...
        <div class="form-group">
          <label class="form-label">ObservaÃ§Ãµes do Contrato</label>
          <textarea class="form-control" id="cContratoObs" rows="2" placeholder="CondiÃ§Ãµes especiais, garantias, SLAs...">${Utils.escHtml(prop.observacoes||'')}</textarea>
        </div>

        <div style="text-align:right;margin-top:8px">
          <button class="btn btn-primary btn-sm" onclick="Propostas._ctab('proj')">PrÃ³ximo: Projeto & Escopo â†’</button>
        </div>
      </div>
      ...
      <div id="cpanel-proj" style="display:none">
        <div class="form-row">
          <div class="form-group">
            <label class="form-label">Ordem de ServiÃ§o (OS)</label>
          ...
          <div class="form-group">
            <label class="form-label">CÃ³digo do Projeto</label>
        ...
        <div class="form-row">
          <div class="form-group">
            <label class="form-label">ResponsÃ¡vel</label>
            <select class="form-control" id="cProjResp"><option value="">â€”</option>${respOpts}</select>
          </div>
          <div class="form-group">
        ...
        <div class="flex items-center justify-between mb-2 mt-2">
          <div class="font-bold text-sm">ðŸ“Œ Etapas do Projeto</div>
        ...
        <div style="background:#fef9c3;border-radius:var(--radius);padding:10px 14px;font-size:13px;margin-top:8px;color:#854d0e">
          âœ… <strong>Criar atividade:</strong> lanÃ§a a etapa no mÃ³dulo de Atividades/Agenda com as datas definidas.<br>
          ðŸ’° <strong>Vinc. Pagamento:</strong> cria um recebÃ­vel no financeiro do projeto com a data fim da etapa como vencimento.
        </div>
      </div>
    `;
  }
    ...
    container.innerHTML = _etapasContrato.map((e, i) => `
      <div style="background:var(--bg);border-radius:var(--radius);padding:12px;margin-bottom:10px;border-left:3px solid var(--primary)">
        <div class="flex items-center justify-between mb-2">
          ...
          <button class="btn btn-xs btn-danger" type="button" onclick="Propostas._removeEtapaContrato(${i})">âœ•</button>
        </div>
        <div class="form-row" style="margin:0 0 8px 0">
          <div class="form-group" style="flex:3;margin-bottom:0">
            ...
            <input class="form-control ec-nome" data-idx="${i}" value="${Utils.escHtml(e.nome)}"
              placeholder="Ex: Levantamento de campo, ElaboraÃ§Ã£o do laudo..."
          ...
          <div class="form-group" style="margin-bottom:0">
            ...
            <select class="form-control" onchange="Propostas._setEtapaCampo(${i},'responsavel',this.value)">
              <option value="">â€”</option>${respOpts}
            </select>
          </div>
        </div>
        <div class="form-row" style="margin:0 0 8px 0">
  ...
  function _confirmarContrato(propostaId) {
    ...
    if (!projTitulo) { Toast.error('Informe o tÃ­tulo do projeto'); _ctab('proj'); return; }
    ...
    // ---- 2. LanÃ§ar recebimentos no financeiro do projeto ----
    ...
    if (condicao === 'avista') {
      ...
      const rec = { id: _uid(), descricao: 'Pagamento Ãºnico', valor: valorFechado,
                    vencimento: data, formaPagamento: forma, status: 'pendente',
                    lancadoFinanceiro: false, recebiveisId: null };
      // Auto-lanÃ§a em recebiveis geral
      const rv = DB.create('recebiveis', {
        descricao: `${projTitulo} â€” Pagamento Ãºnico`,
      ...
      for (let i = 0; i < n; i++) {
        ...
        const rec = { id: _uid(), descricao: descParcela, valor: parcVal,
                      ...
                      lancadoFinanceiro: false, recebiveisId: null };
        const rv = DB.create('recebiveis', {
          descricao: `${projTitulo} â€” ${descParcela}`,
      ...
      // Vinculado a etapas â€” cria recebÃ­vel para cada etapa com vincPagamento = true
      etapasValidas.filter(e => e.vincPagamento && e.valor > 0).forEach((e, i) => {
        const descRec = `MediÃ§Ã£o â€” ${e.nome.trim()}`;
        ...
        const rv = DB.create('recebiveis', {
          descricao: `${projTitulo} â€” ${descRec}`,
    ...
    etapasValidas.filter(e => e.criarAtividade).forEach(e => {
      DB.create('atividades', {
        ...
        descricao: `Etapa do projeto ${projTitulo} â€” ${e.nome.trim()}`,
        origem: 'contrato',
      });
      atividadesCriadas++;
    });
    ...
    Toast.success(
      `ðŸŽ‰ Contrato fechado! Projeto criado, ` +
      `${recCount} recebÃ­vel(is) lanÃ§ado(s) e ` +
  ...
  function drillDown(tipo) {
    ...
    const getCliente = p => DB.get('clientes', p.clienteId)?.nome || p.clienteNome || 'â€”';
